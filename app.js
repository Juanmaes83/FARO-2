...
function wireForegroundStages(){const pairs=$$('.sec .fg').map(stage=>({section:stage.closest('.sec'),stage})).filter(p=>p.section);if(!pairs.length)return;const sky=$('#fg-sky'),homes=new WeakMap(pairs.map(p=>[p.stage,p.section])),timers=new WeakMap();let current=null,ticking=false;
