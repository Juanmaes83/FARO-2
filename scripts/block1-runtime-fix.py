...
new = """function wireForegroundStages(){const pairs=$$('.sec .fg,.foot .fg').map(stage=>({section:stage.closest('.sec,.foot'),stage})).filter(p=>p.section);if(!pairs.length)return;const sky=$('#fg-sky'),homes=new WeakMap(pairs.map(p=>[p.stage,p.section])),timers=new WeakMap();let current=null,ticking=false;
 const lift=stage=>{if(!sky||stage.parentNode===sky)return;sky.appendChild(stage);void stage.offsetWidth};
 const park=stage=>{const h=homes.get(stage);if(h&&stage.parentNode!==h)h.insertBefore(stage,h.firstChild)};
 ...
 const resolve=()=>{ticking=false;const vh=innerHeight,mid=vh*.5;let best=null,bestScore=-1;for(const p of pairs){const r=p.section.getBoundingClientRect(),visible=Math.max(0,Math.min(r.bottom,vh)-Math.max(r.top,0));if(visible<=0)continue;const coverage=visible/Math.max(1,Math.min(r.height,vh)),center=1-Math.min(1,Math.abs((r.top+r.height*.5)-mid)/vh),score=coverage*.78+center*.22;if(score>bestScore){bestScore=score;best=p}}if(best)activate(best.stage);else if(current){const old=current;current=null;retire(old)}};
...
if old not in s:
    raise SystemExit('Expected foreground lifecycle block not found; refusing unsafe patch')

p.write_text(s.replace(old, new, 1), encoding='utf-8')
print('app.js foreground lifecycle replaced successfully')
